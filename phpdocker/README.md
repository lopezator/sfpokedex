...
You can access your application via **`localhost`**, if you're running the containers directly, or through **`192.168.33.135`** when run on a vm. nginx and mailhog both respond to any hostname, in case you want to add your own hostname on your `/etc/hosts` 

Service|Address outside containers|Address outside VM
------|---------|-----------
Webserver|[localhost:8080](http://localhost:8080)|[192.168.33.135](http://192.168.33.135)
...
------|---------|-----------
php-fpm| []() sfpokedex-php-fpm|9000
MySQL|sfpokedex-mysql|3306 (default)
...
**Note 2:** if you're using Vagrant, you'll need to ssh into it first
  ...
  * Execute command inside of container: `docker exec -it sfpokedex-php-fpm COMMAND` where `COMMAND` is whatever you want to run. For instance, `/bin/bash` to open a console prompt.
